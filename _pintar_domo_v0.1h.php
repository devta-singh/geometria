...
//Escala
if(!isset($_REQUEST["escalaz"])){
	$escalaz = 1;
}else{
	$escalaz = $_REQUEST["escalaz"];
}
...
//margen_arriba2
if(!isset($_REQUEST["margen_arriba2"])){
	$margen_arriba2 = 100;
}else{
	$margen_arriba2 = $_REQUEST["margen_arriba2"];
}
...
<br>
/ Margen superior alzado: 
<input type="text" name="margen_arriba2" value="$margen_arriba2" size="5"/>
<br>
...
Escala Z:<input type="text" name="escalaz" value="$escalaz" size="5"/> / 

Escala notación:<input type="text" name="escala_notacion" value="$escala_notacion" size="5"/>
...
if(!isset($_REQUEST["centrar2"])){
	$centro_y2 = 0;
}else{
	if($_REQUEST["centrar2"] == 1){
		$centro_y2 = $alto / 2;
	}elseif($_REQUEST["centrar2"] == 2){
		$centro_y2 = $margen_arriba2;
	}
}
...
function pinta_segmentos_alzado($im, $nodos, $color, $color2="", $escala=0){
	if(!$color2){
		$color2 = $color;
	}
	...
	if(!isset($_REQUEST["centrar"])){
		...
		$centro_x=imagesx($im) / 2;
		$centro_y=imagesy($im) / 2;
		$centro_z =imagesy($im) / 2 + 200;
	}elseif($_REQUEST["centrar"]==2){
		$centro_x=$_REQUEST["margen_izquierdo"];
		$centro_y=$_REQUEST["margen_arriba"];
		$centro_z = $_REQUEST["margen_arriba2"];
	}

	if(!isset($_REQUEST["centrar2"])){
		$centro_y2 = 0;
	}elseif($_REQUEST["centrar2"]==1){
		$centro_y2=imagesy($im) / 2;
	}elseif($_REQUEST["centrar2"]==2){
		$centro_y2=$_REQUEST["margen_arriba2"];
	}
	...
	for($ni=0; $ni < $n_niveles; $ni++){
		...
		for($no=0; $no < $n_nodos; $no++){
			...
			//pintamos el segmento AB
			imageline($im, ($centro_x + ($Ax * $escalax)), ($centro_y2 + ($Az * $escalaz)), ($centro_x + ($Bx * $escalax)), ($centro_y2 + ($Bz * $escalaz)), $color);
			...
			//pintamos el segmento AC
			imageline($im, ($centro_x + ($Ax * $escalax)), ($centro_y2 + ($Az * $escalaz)), ($centro_x + ($Cx * $escalax)), ($centro_y2 + ($Cz * $escalaz)), $color2);
			
			//pintamos el segmento BC
			imageline($im, ($centro_x + ($Bx * $escalax)), ($centro_y2 + ($Bz * $escalaz)), ($centro_x + ($Cx * $escalax)), ($centro_y2 + ($Cz * $escalaz)), $color2);
...
$segmentos_alzado = pinta_segmentos_alzado($im, $nodos, $azul, $rojo);
